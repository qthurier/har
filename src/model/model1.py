...
class TimeAndFeatWiseNormalisation(Layer):
    ...
    def __init__(self, n_feat: int, feat_block_size: int, **kwargs: Any) -> None:
        ...
        self.blocks = [
            Sequential(
                [ExtractFeatRange(i, i + self.feat_block_size), LayerNormalization()],
                name=f"block{1}",
            )
            for i in range(0, self.n_feat, self.feat_block_size)
        ]

    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        timewise_norm_inputs = TimeWiseNormalisation()(inputs)
        featwise_norm_inputs_blocks = [block(inputs) for block in self.blocks]
        norm_inputs = Concatenate()(
            [timewise_norm_inputs] + featwise_norm_inputs_blocks
        )
        return norm_inputs
...
class Classifier(tf.keras.Model):
        ...
        **kwargs: Any,
        ...
        self.rnns = [
            Sequential(
                [
                    Bidirectional(
                        GRU(encoder_dim, return_sequences=True, activation="relu"),
                        merge_mode="sum",
                    ),
                    LayerNormalization(),
                ],
    ...
    def call(self, inputs) -> tf.Tensor:
        ...
        local_cnn_representation = Concatenate()(
            [self.local_cnns[0](norm_inputs), self.local_cnns[1](inputs)]
        )
        global_cnn_representation = Concatenate()(
            [self.global_cnns[0](norm_inputs), self.global_cnns[1](inputs)]
        )
        rnn_representation = Concatenate()(
            [self.rnns[0](norm_inputs), self.rnns[1](inputs)]
        )
        mixed_representation = Concatenate()(
            [
                self.denses[0](local_cnn_representation),
                self.denses[1](global_cnn_representation),
                self.denses[2](rnn_representation),
            ]
        )
