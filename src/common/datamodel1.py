...
def _parse_raw_ts_file(file: str) -> tf.data.Dataset:
    """Turn a raw inertial signal file into a tensorflow dataset"""
    return (
...
def _parse_folder(parser: Callable, folder: str) -> tuple[tf.data.Dataset]:
    """Parse a whole folder of raw inertial signal files"""
    return tuple(parser(file) for file in sorted(Path(folder).glob("*")))


def _keep_subjects(subjects: list[int]) -> Callable:
    """Closure that filter records within a tensorflow dataset creation pipeline"""
...
def make_labelled_dataset(
    ...
    **kwargs: Any
    ...
    """Create a tensorflow dataset containing features and labels, used equally to build validation, training and test sets"""
    ...
    multivariate_ts = tf.data.Dataset.zip(univariate_ts).map(
        lambda *t: tf.stack(t, axis=-1)  # concatenate all the time series tensors
    )
...
def get_labels(labelled_dataset: tf.data.Dataset) -> list[int]:
    """Extract labels from a dataset generated by make_labelled_dataset"""
...
def get_subjects(file: str) -> set[int]:
    """Turn a subjects text file into a set of subjects"""
